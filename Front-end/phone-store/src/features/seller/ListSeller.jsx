...
import { FaUsers, FaSyncAlt, FaSpinner } from 'react-icons/fa';
  ...
  if (loading) {
    return (
      <div className="w-full h-screen flex flex-col justify-center items-center bg-gray-50">
        ...
        <p className="text-gray-600 text-lg">Carregando vendedores...</p>
      </div>
    );
  }
    ...
    return (
      <div className="w-full h-screen flex flex-col justify-center items-center bg-gray-50 text-gray-700">
        <div className="bg-white p-8 rounded-lg shadow-lg text-center max-w-md border border-gray-200">
          ...
          >
            <FaSyncAlt className="mr-2 w-4 h-4" /> Tentar Novamente
          </button>
        </div>
      </div>
    );
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
          <h1 className="text-2xl font-bold text-gray-700 flex items-center mb-4 sm:mb-0">
            <FaUsers className="mr-3 w-6 h-6 text-gray-500" />
            Lista de Vendedores ({sellers.length})
          </h1>
          ...
          >
            <FaSyncAlt className="mr-2 w-4 h-4" /> Atualizar Lista
          </button>
        </div>
        ...
        {sellers.length === 0 ? (
          <div className="p-6 text-center bg-gray-100 border border-gray-300 text-gray-600 rounded-lg shadow-sm">
            <p className="font-bold">Nenhum Vendedor Encontrado</p>
            <p>A lista de vendedores está vazia. Certifique-se de que o Back-end está rodando e autenticado.</p>
          </div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <TableHead text="Situação" />
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-100">
                {sellers.map((seller) => (
                  <tr key={seller.sellerId} className="hover:bg-gray-50 transition duration-100">
